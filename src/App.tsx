...
type StatusFilter = 'all' | 'pending' | 'passportPending' | 'enrollmentPending' | 'completed';
...
const matchesStatusFilter = (applicant: Applicant, statusFilter: StatusFilter) => {
  const passportPending = !applicant.passportSubmitted;
  const enrollmentPending = !applicant.enrollmentSubmitted;

  switch (statusFilter) {
    case 'pending':
      return passportPending || enrollmentPending;
    case 'passportPending':
      return passportPending;
...
function App() {
  ...
  const filteredApplicants = useMemo(() => {
    ...
    return applicants.filter((applicant) => {
      ...
      return matchesSearch && matchesStatusFilter(applicant, statusFilter);
    });
  ...
  return (
    <main className="container">
      ...
      <section className="filters">
        ...
        <label>
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}>
            ...
            <option value="passportPending">パスポートコピー未提出</option>
            <option value="enrollmentPending">在籍証明書未提出</option>
      ...
      {filteredApplicants.length ? (
        ...
        <p className="empty-message">該当する応募者はいません。</p>
      )}
    </main>
  );
}
